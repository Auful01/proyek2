            ...
            <div class="collapse navbar-collapse" id="navbarNav">
                <ul class="navbar-nav ml-auto">
                    ...
                    @if (auth()->check())
                        ...
                        <li class="nav-item {{ Request::segment(1) == 'market-pengepul' ? 'active' : '' }}">
                            <a class="nav-link" href="{{ route('market.pengepul') }}">Market Pengepul</a>
                            </li>
                        @endif
                        @if (auth()->user()->role == 'pgl')
                            <li class="nav-item {{ Request::segment(1) == 'market-penjual' ? 'active' : '' }}">
                                <a class="nav-link" href="{{ route('market.penjual') }}">Market Penjual</a>
                            </li>
                        @endif
                        @endif
                    @if (auth()->check())
                    <li class="nav-item mx-md-2 my-md-0 my-sm-2 my-2">
                        <a class="nav-link btn btn-sm btn-danger text-light "
                        href="" onclick="event.preventDefault(); document.getElementById('frm-logout').submit();">Logout</a>
                    ...
                    <li class="nav-item mx-md-2 my-md-0 my-sm-2 my-2">
                        <a class="nav-link btn btn-sm btn-outline-success "
                            href="{{ route('user.show.login') }}">Login</a>
                    </li>
                    <li class="nav-item mx-md-2 my-md-0 my-sm-2 my-2">
                        <a class="nav-link btn btn-sm btn-success text-light"
                            href="{{ route('user.show.register') }}">Register</a>
                    </li>
                    @endif
                </ul>
            </div>
