...
@section('content')
    <div class="page-body">
        <div class="container-fluid">
            ...
            <div class="row mb-3">
                <div class="col-12">
                    <a href="{{ route('admin.barang.create') }}" class="btn btn-sm btn-primary">Tambah</a>
                </div>
                    ...
                    <table id="example" class="table table-striped table-bordered dt-responsive nowrap mt-5"
                        ...
                        <tbody>
                            @foreach ($data as $item)
                                <tr>
                                    <td>{{$loop->index + 1}}</td>
                                    <td>{{$item->junk_category->name}}</td>
                                    <td>{{$item->name}}</td>
                                    <td>{{$item->weight}}</td>
                                    <td>{{$item->price}}</td>
                                    <td>
                                        <center>
                                            <a href="{{ route('admin.barang.edit', $item->id) }}" class="btn btn-warning btn-sm">Ubah</a>
                                            <form style="display: inline-block;" id="myForm" action="{{ route('admin.barang.destroy', $item->id) }}" method="post">
                                                {{ csrf_field() }}
                                                {{ method_field('DELETE') }}
